           ...
           <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {TESTIMONIALS.map((review) => (
                  <div key={review.id} className="bg-white/80 dark:bg-slate-800/40 backdrop-blur-md p-8 rounded-2xl border border-slate-200 dark:border-slate-700 hover:border-emerald-500/30 transition-all duration-300 relative group shadow-sm dark:shadow-none">
                    <Quote className="absolute top-6 left-6 text-slate-200 dark:text-slate-700 w-10 h-10 group-hover:text-emerald-500/20 transition-colors" />
                    ...
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      />
                      <div>
                        <h4 className="text-slate-900 dark:text-white font-bold transition-colors duration-300">{review.name}</h4>
                        <p className="text-sm text-slate-500 dark:text-gray-400 transition-colors duration-300">{review.role}</p>
                      </div>
                    </div>
                    
                    <p className="text-slate-600 dark:text-gray-300 italic leading-relaxed relative z-10 transition-colors duration-300">
                      "{review.content}"
                    </p>
                    
                    <div className="mt-6 flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <span key={star} className="text-pink-600 dark:text-pink-500 text-sm">★</span>
                      ))}
                    </div>
                  </div>
                ))}
